...
**2026.06.07** — v2.9.6
- Shopping：修复 `/gNN`(g20)扫描结束后,**原生拍卖行的一口价排序被留在降序**(最贵的排最前),导致之后日常(非 g20)随手搜东西也跟着"最贵的先显示"、得手动去原生 UI 再点一次一口价列才恢复的问题。现在 `/gNN` 扫描收尾时自动把原生排序还原成升序(等于普通搜索结束后的状态);g20 扫描期间仍按降序把大堆叠抢到前几页(该加速功能不变),只是收尾后不再把降序状态遗留给原生界面。只有 g20 才触发还原,日常使用与加该功能之前完全一致。
