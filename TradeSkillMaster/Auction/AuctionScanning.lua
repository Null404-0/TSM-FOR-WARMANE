...
--    doCache - whether to cache results
--    fastScan - 【新增】快速扫描模式：只扫第一页（压价优化）
function TSMAPI.AuctionScan:RunQuery(query, callbackHandler, resolveSellers, maxPrice, doCache, fastScan)
	...
	elseif not CanSendAuctionQuery() then
		TSMAPI:CreateTimeDelay("cantSendAuctionQueryDelay", 0.1, function() TSMAPI.AuctionScan:RunQuery(query, callbackHandler, resolveSellers, maxPrice, doCache, fastScan) end)
...
function private:ScanAuctions()
	...
	-- fire page update AFTER records are added so data is current
	if private.scanType ~= "lastPage" and totalPages > 0 then
		DoCallback("SCAN_PAGE_UPDATE", private.query.page, totalPages, private.data)
	end
...
function private:AddAuctionRecord(index)
	...
	highBidder = TSM:GetAuctionPlayer(highBidder, nil)
...
local function CompareTableKeys(tbl1, tbl2)
	for _, key in ipairs(findPrivate.keys) do
		-- treat "?" as wildcard for seller (unresolved seller names during scan)
		if key == "seller" and (tbl1[key] == "?" or tbl2[key] == "?") then
			-- skip seller comparison
		elseif tbl1[key] ~= tbl2[key] then
			return
		end
	end
	return true
