...
L["Below Custom Price ('0c' to disable)"] = "低于自定义价格（设为 '0c' 则禁用）"
L["Below Max Price"] = "低于最高价"
L["Below Vendor Sell Price"] = "低于 NPC 出售价"
L["Bid Percent"] = "竞价百分比"
L["Buyout"] = "一口价"
...
L["If checked, the maximum shopping price will be shown in the tooltip for the item."] = "如果勾选，物品的最高购买价格将显示在鼠标提示中。"
...
L["Items which are below their maximum price (per their group / Shopping operation) will be displayed in Sniper searches."] = "低于其最高价格（由所属分组/Shopping 操作决定）的物品将显示在狙击搜索中。"
L["Items which are below their vendor sell price will be displayed in Sniper searches."] = "低于 NPC 出售价格的物品将显示在狙击搜索中。"
L["Items which are below this custom price will be displayed in Sniper searches."] = "低于此自定义价格的物品将显示在狙击搜索中。"
...
L["NOTE: The scan must be stopped before you can buy anything."] = "注意：必须停止扫描才能购买物品。"
L["Num"] = "数量"
...
L["Show Shopping Max Price in Tooltip"] = "在鼠标提示中显示 Shopping 最高价格"
...
L["The disenchant search looks for items on the AH below their disenchant value. You can set the maximum percentage of disenchant value to search for in the Shopping General options"] = "分解搜索会在拍卖行中查找低于其分解价值的物品。你可以在 Shopping 常规选项中设置要搜索的最大分解价值百分比。"
...
L["The Sniper feature will look in real-time for items that have recently been posted to the AH which are worth snatching! You can configure the parameters of Sniper in the Shopping options."] = "狙击功能将实时查找最近发布到拍卖行且值得抢购的物品！你可以在 Shopping 选项中配置狙击参数。"
...
L["This is the maximum percentage of disenchant value that the Other > Disenchant search will display results for."] = "这是 Other（其他）> Disenchant（分解）搜索显示结果时所使用的最大分解价值百分比。"
...
L["Click to search for an item.\nShift-Click to post at market value."] = "单击物品 将扫描拍卖\nShift+单击 将以列出的市场价格来发布拍卖"
L["Inline Filters:|r You can easily add common search filters to your search such as rarity, level, and item type. For example '%sarmor/leather/epic/80/i200/i285|r' will search for all leather armor of epic quality that requires level 80 and has an ilvl between 200 and 285 inclusive. Also, '%sinferno ruby/exact|r' will display only raw inferno rubys (none of the cuts)."] = "|cffffff00在线过滤:|r 你可以通过添加 稀有度,等级,物品类型 来过滤物品。例如 '%sarmor/leather/epic/80/i200/i285|r' 将会查找所有皮甲并且满足(史诗级，80级，物品等级在200~285。再如, '%sinferno ruby/exact|r' 将只会显示原始的地狱炎石而不包括切割后的产品。"
L["Max Disenchant Level"] = "最高分解等级"
L["Min Disenchant Level"] = "最低分解等级"
L["Multiple Search Terms:|r You can search for multiple things at once by simply separated them with a ';'. For example '%scopper ore; gold ore|r' will search for both copper and gold ore."] = "|cffffff00多重查询条件:|r 你可以一次查询多个物品通过使用';'符号分割开来。例如 '%s铜矿石; 金矿石|r' 将会同时查找铜矿石和金矿石。"
L["Posted a %s with a buyout of %s."] = "以一口价 %s, 发布拍卖 %s。"
L["This is the maximum item level that the Other > Disenchant search will display results for."] = "这是 Other（其他）> Disenchant（分解）搜索显示结果的最高物品等级。"
