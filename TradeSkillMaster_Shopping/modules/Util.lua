...
function Util:SetParent(parent)
    if private.parent ~= parent then
        private.searchFrame = nil  -- 父级变了，下次重新创建 frame
    end
...
function private.ScanCallback(event, ...)
	...
	elseif event == "SCAN_PAGE_UPDATE" then
		...
		private:UpdateStatus("page", page, totalPages)
		if pageData and private.filterList and private.filterList[1] then
			for itemString, auctionData in pairs(pageData) do
				if not auctionData.isBaseItem then
					if private.auctions[itemString] then
						-- 已存在：重新调用 callback("process") 过滤新追加的记录
...
function private:ScanComplete()
	if not private.callback then
		-- 【修复】即使没有callback，也要清理其他状态
		private.filterList = nil
		private.searchItem = nil
		return
	end
