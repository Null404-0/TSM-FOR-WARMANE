...
L[" Disable Destroying Search"] = " 禁用分解搜索"
L["AH"] = "拍卖行"
L["All"] = "全部"
...
L["Crafting will not queue any items affected by this operation with a profit below this value. As an example, a min profit of 'max(10g, 10% crafting)' would ensure a profit of at least 10g or 10% of the craft cost, whichever is highest."] = "此操作影响的物品若利润低于此值，Crafting 将不会把它们加入队列。例如，最低利润设为 'max(10g, 10% crafting)' 会确保至少 10 金或制造成本 10% 的利润（取较高者）。"
L["Crafts"] = "配方"
L["Craft Next"] = "制造下一个"
L["Craft Price Method"] = "制造价格设定"
L["Craft Queue"] = "制造队列"
L["Craft Value Method"] = "制造价值计算方法"
L["Created profession group for %s."] = "已为 %s 创建专业分组。"
L["Create Profession Groups"] = "创建一个专业分组"
L["Current Source"] = "当前来源"
...
L["Default Craft Value Method"] = "默认制造价值计算方法"
...
L["If checked, only a normal AH search will be performed"] = "如果勾选，仅执行普通的拍卖行搜索。"
L["If checked, only crafts which you can craft with items in your bags (ignoring vendor items) will be shown below."] = "如果勾选，下方仅显示你可以用背包中物品（忽略 NPC 出售物品）制造的配方。"
L["If checked, the AH destroying search will only look for even stacks"] = "如果勾选，拍卖行分解搜索仅查找偶数堆叠。"
...
L["If checked, the mats needed to craft an item and their prices will be shown in item tooltips."] = "如果勾选，制造物品所需的材料及其价格将显示在物品的鼠标提示中。"
...
L["Link"] = "链接"
L["List Mats in Tooltip"] = "在鼠标提示中列出材料"
...
L["Override Default Craft Value Method"] = "覆盖默认的制造价值计算方法"
...
L["Profession data not found for %s on %s. Logging into this player and opening the profession may solve this issue."] = "在 %s 上未找到 %s 的专业数据。登录该角色并打开其专业界面可能解决此问题。"
...
L["Profession Frame Scale"] = "专业窗口缩放"
L["Professions"] = "专业"
...
L["The scale of the profession frame."] = "专业窗口的缩放比例。"
...
L["This is the default method Crafting will use for determining the value of crafted items."] = "这是 Crafting 默认用于计算制造物品价值的方法。"
L["This slider sets the quantity to add/remove from the queue when left/right clicking on a row below."] = "此滑块设置在下方表格中左键/右键单击某一行时向队列添加或移除的数量。"
L["Total"] = "总计"
L["Transform"] = "转化"
L["TSM Groups"] = "TSM分组"
L["Vendor"] = "NPC"
...
L["You can left/right click on a row to add/remove a craft from the crafting queue."] = "你可以左键/右键单击某一行来从制造队列中添加或移除一个制造项目。"
L["Your craft value method for '%s' was invalid so it has been returned to the default. Details: %s"] = "你为 '%s' 设置的制造价值计算方法无效，因此已恢复为默认值。详情：%s"
L["Your default craft value method was invalid so it has been returned to the default. Details: %s"] = "你的默认制造价值计算方法无效，因此已恢复为默认值。详情：%s"
