...
function GUI:EventHandler(event, ...)
	...
	if unittest == "player" or unittest==nil then --Changing tradeskill frames and stuff has "nil" unit, when other players cast this also triggers with nil
		...
		elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
			...
			-- SpellName2ID uses English names; on Chinese client spellName is localized.
			-- Fall back to matching against the spell we are currently crafting.
			if spellID == nil and GUI.isCrafting then
				local craftingSpellName = GetSpellInfo(GUI.isCrafting.spellID)
				if craftingSpellName and craftingSpellName == spellName then
					spellID = GUI.isCrafting.spellID
				end
			end
			...
			-- Decrement queue count only when we have a confirmed match
			...
			if GUI.isCrafting and spellID == GUI.isCrafting.spellID then
				...
				if GUI.isCrafting.quantity <= 0 then
					GUI.isCrafting = nil
					-- Use 0.5s delay to allow ItemTracker to update bag counts first
					TSMAPI:CreateTimeDelay("craftingQueueUpdateThrottle", 0.5, GUI.UpdateQueue)
				end
			end
		...
		elseif event == "UNIT_SPELLCAST_INTERRUPTED" or event == "UNIT_SPELLCAST_FAILED" or event == "UNIT_SPELLCAST_FAILED_QUIET" then
			...
			if GUI.isCrafting then
				local spellID = TSM.SpellName2ID[spellName]

				-- Same localized name fallback as SUCCEEDED
				if spellID == nil then
					local craftingSpellName = GetSpellInfo(GUI.isCrafting.spellID)
					if craftingSpellName and craftingSpellName == spellName then
						spellID = GUI.isCrafting.spellID
				...
				if spellID == GUI.isCrafting.spellID then
					GUI.isCrafting = nil
					TSMAPI:CreateTimeDelay("craftingQueueUpdateThrottle", 0.2, GUI.UpdateQueue)
				end
			end
...
function GUI.CraftWatchdog()
	...
	-- 批量制作两次施法之间并非无缝:客户端要等服务器确认上一次成功才发起下一次,空档约等于
	-- 网络延迟,卡顿时可能超过 1 秒。v2.9.6 用 0.6s 判停会把这种空档误判成"已停止",提前
	-- 清掉 isCrafting,造成本地化客户端队列漏扣、按钮提前解锁,连点之下多做越界。统一用 2s:
	-- 正常批间空档远碰不到;被移动打断后,事件名匹配得上的打断由 UNIT_SPELLCAST_INTERRUPTED
	-- 即时恢复,这里只兜匹配不上的底,最多 2 秒恢复按钮,仍远好于重登。
	if GUI.craftIdleTime >= 2 then
		-- 恢复按钮前先撤销可能残存的批量重复(客户端提供该 API 时),
		-- 避免误判"已停止"时旧批量与下一次点击的 DoTradeSkill 叠加。
...
function GUI:CreateQueueFrame(parent)
	...
	btn:SetScript("OnClick", function(self)
		...
		-- 自己的批量还没结束(isCrafting 尚未被完成/打断事件或看门狗清掉)就不再发
		-- DoTradeSkill,防止连点器踩进两次施法之间的空档把第二批叠上去(多做越界)。
		-- isCrafting 由事件即时清除,看门狗 2 秒兜底清除,不会把按钮锁死。
		if GUI.isCrafting then return end
